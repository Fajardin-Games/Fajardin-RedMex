...
class OriginatingMessageManager(
        ...
        private val _state =
                MutableStateFlow<Map<Int, VirtualNode.LastOriginatorMessage>>(emptyMap())

        val state: Flow<Map<Int, VirtualNode.LastOriginatorMessage>> = _state.asStateFlow()

        private val receivedMessages: Flow<VirtualNode.LastOriginatorMessage> =
                MutableSharedFlow(
                        replay = 1,
        ...
        data class PendingPing(val ping: MmcpPing, val toVirtualAddr: Int, val timesent: Long)
        ...
        private val sendOriginatingMessageRunnable = Runnable {
                ...
                val packet =
                        originatingMessage.toVirtualPacket(
                ...
                val neighbors = originatorMessages.filter { it.value.hopCount == 1.toByte() }
                ...
                // check if we have an active station connection but have lost the originating
                // message from
                ...
                if (stationNeighborInetAddr != null &&
                                !neighbors.any {
                                        it.value.lastHopRealInetAddr == stationNeighborInetAddr
                                } &&
                        ...
                        logger(
                                Log.WARN,
                                "$logPrefix : sendOriginatingMessagesRunnable: have not received " +
                        ...
                        try {
                                ...
                                logger(
                                        Log.ERROR,
                                        "$logPrefix : sendOriginatingMessagesRunnable: could not " +
                                                "send originating message to group owner",
                                        e
                                )
                        }
                } else if (stationNeighborInetAddr != null &&
                                stationState.stationBoundDatagramSocket == null
                ) {
                        logger(
                                Log.WARN,
                                "$logPrefix : sendOriginatingMessagesRunnable : could not send " +
        ...
        private val pingNeighborsRunnable = Runnable {
                ...
                neighbors.forEach {
                        ...
                        logger(
                                priority = Log.VERBOSE,
                                message = {
                                        "$logPrefix pingNeighborsRunnable: send ping to ${neighborVirtualAddr.addressToDotNotation()}"
                                }
                        )
        ...
        private val checkLostNodesRunnable = Runnable {
                ...
                val nodesLost =
                        originatorMessages.entries.filter {
                                (timeNow - it.value.timeReceived) >
                                        originatingMessageNodeLostThreshold
                        }
        ...
        private val sendOriginatorMessagesFuture =
        ...
        private val pingNeighborsFuture =
                scheduledExecutorService.scheduleAtFixedRate(
        ...
        private val checkLostNodesFuture =
                scheduledExecutorService.scheduleAtFixedRate(
        ...
        private fun assertNotClosed() {
                if (closed) throw IllegalStateException("$logPrefix is closed!")
        }
        ...
        fun onReceiveOriginatingMessage(
                ...
                val connectionPingTime =
                        neighborPingTimes[virtualPacket.header.lastHopAddr]?.pingTime ?: 0
                ...
                // Update this only if it is more recent and/or better. It might be that we are
                // getting it
                // back
                // via some other (suboptimal) route with more hops
                val currentlyKnownSentTime =
                        (currentOriginatorMessage?.originatorMessage?.sentTime ?: 0)
                ...
                val isMoreRecentOrBetter =
                        mmcpMessage.sentTime > currentlyKnownSentTime ||
                                mmcpMessage.sentTime == currentlyKnownSentTime &&
                                        virtualPacket.header.hopCount < currentlyKnownHopCount
                val isNewNeighbor =
                        virtualPacket.header.hopCount == 1.toByte() &&
                ...
                if (currentOriginatorMessage == null || isMoreRecentOrBetter) {
                        originatorMessages[virtualPacket.header.fromAddr] =
                                VirtualNode.LastOriginatorMessage(
                                        originatorMessage =
                                                mmcpMessage.copyWithPingTimeIncrement(
                                                        connectionPingTime
                                                ),
        ...
        fun onPongReceived(
                ...
                val pendingPingPredicate: (PendingPing) -> Boolean = {
                        it.ping.messageId == pong.replyToMessageId &&
                                it.toVirtualAddr == fromVirtualAddr
                }
                ...
                if (pendingPing == null) {
                        logger(
                                Log.WARN,
                                "$logPrefix : onPongReceived : pong from " +
                ...
                neighborPingTimes[fromVirtualAddr] =
                        PingTime(
        ...
        fun lookupNextHopForChainSocket(address: InetAddress, port: Int): ChainSocketNextHop {
                ...
                return when {
                        ...
                        // Destination is a direct neighbor (final destination) - connect to the
                        // actual socket
                        // itself
                        ...
                        originatorMessage != null -> {
                                ChainSocketNextHop(
                                        originatorMessage.lastHopRealInetAddr,
                                        originatorMessage.lastHopRealPort,
                                        false,
                                        originatorMessage.receivedFromSocket.boundNetwork
                                )
                        }
        ...
        /**
         * Run the process to add a new neighbor (e.g. after a Wifi station connection is
         * established).
         *
         ...
         * @param neighborRealPort The port on which the neighbor is running
         * VirtualNodeDatagramSocket
         * @param socket our VirtualNodeDatagramSocket through which we will attempt to communicate
         * with
         * ```
         ...
         * @param timeout
         * ```
         * the timeout (in ms) for the new connection to be established. If the timeout
         * ```
         *        is exceeded an exception will be thrown
         * @param sendInterval
         * ```
         * the interval period for sending out originating messages to the new neighbor
         */
        suspend fun addNeighbor(
                ...
                // send originating packets out to the other device until we get something back from
                // it
                val sendOriginatingMessageJob =
                        scope.launch {
                                while (isActive) {
                                        try {
                                                val originatingMessage = makeOriginatingMessage()
                ...
                try {
                        withTimeout(timeout.toLong()) {
                                val replyMessage =
                                        receivedMessages
                                                .filter {
                                                        it.lastHopRealInetAddr ==
                                                                neighborRealInetAddr &&
                                                                it.lastHopRealPort ==
                                                                        neighborRealPort
                                                }
                                                .first()
                                logger(
                                        Log.DEBUG,
                                        "$logPrefix addNeighbor - received originating message reply " +
