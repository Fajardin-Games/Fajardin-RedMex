...
class App: Application(), DIAware {
    ...
    private val diModule = DI.Module("meshrabiya-module") {
        ...
        bind<File>(tag = TAG_CHAT_HISTORY_FILE) with singleton {
            File(filesDir, "chat_history.json")
        }

        bind<File>(tag = TAG_NICKNAMES_FILE) with singleton {
            File(filesDir, "nicknames.json")
        }

        bind<ContactsManager>() with singleton {
            ContactsManager(
                nicknamesFile = instance(tag = TAG_NICKNAMES_FILE),
                json = instance()
            )
        }
        ...
        bind<TestAppServer>() with singleton {
            ...
            TestAppServer(
                ...
                json = instance(),
                chatHistoryFile = instance(tag = TAG_CHAT_HISTORY_FILE)
            )
        }
