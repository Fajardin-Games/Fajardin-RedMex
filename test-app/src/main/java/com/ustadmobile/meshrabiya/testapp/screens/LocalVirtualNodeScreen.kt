    ...
    LazyColumn(
        ...
        item(key = "header") {
            ListItem(
                ...
                supportingContent = {
                    Column {
                        Text("Dirección de malla virtual")
                        ...
                        Row {
                            ...
                            Text(
                                "Esta es su dirección de malla virtual. Se puede usar para llegar a " +
                                        "este nodo desde cualquier otro nodo conectado a la malla, incluso si " +
                                        "no están conectados directamente, por ejemplo, cuando el Dispositivo A está conectado al " +
                                        "Dispositivo B, y el Dispositivo B está conectado al Dispositivo C, el Dispositivo A y " +
                                        "C pueden comunicarse a través de la dirección de malla virtual."
                            )
                ...
                ListItem(
                    ...
                    supportingContent = {
                        Row {
                            ...
                            Text("Esto crea un punto de acceso que otros dispositivos pueden usar para conectarse a " +
                                    "este dispositivo como una estación WiFi (cliente). No compartirá Internet móvil. " +
                                    "Cualquier dispositivo puede operar como punto de acceso WiFi y estación (cliente) " +
                                    "simultáneamente. Una vez creado el punto de acceso, se mostrará un código QR " +
                                    "que se puede usar para conectar dispositivos. Esto no puede iniciarse " +
                                    "si está utilizando el punto de acceso móvil del sistema y/o tethering.")
                        }
                    }
                )
            }
            ...
            item(key = "hotspotstate") {
                Text(
                    ...
                    text = "SSID: ${uiState.wifiState?.connectConfig?.ssid} (${uiState.wifiState?.connectConfig?.band})\n" +
                            "Contraseña: ${uiState.wifiState?.connectConfig?.passphrase}\n" +
                            "LinkLocal: ${uiState.wifiState?.connectConfig?.linkLocalAddr}\n" +
                            "Dirección MAC: ${uiState.wifiState?.connectConfig?.bssid ?: uiState.wifiState?.connectConfig?.linkLocalToMacAddress}\n" +
                            "Puerto: ${uiState.wifiState?.connectConfig?.port}\n"
                )
            }
        }
        ...
        item(key = "stationheader") {
            ListItem(
                headlineContent = {
                    Text("Conexión de estación Wifi (cliente)")
                },
            )
        }
                    ...
                    OutlinedButton(
                        ...
                        onClick = {
                            ...
                            qrCodeScannerLauncher.launch(ScanOptions().apply {
                                ...
                                setPrompt("Habilita el punto de acceso en la aplicación Meshrabiya en el dispositivo " +
                                        "al que deseas conectarte, luego escanea el código QR."
                                )
                                setDesiredBarcodeFormats(ScanOptions.QR_CODE)
                            })
                        },
                    ) {
                        Text("Conectar vía escáner QR")
                    }
                }else {
                    ListItem(
                        headlineContent = {
                            Text(stationState.config?.ssid ?: "(SSID Desconocido)")
                        },
                        supportingContent = {
                            Text(
            ...
            ListItem(
                ...
                supportingContent = {
                    Row {
                        ...
                        Text(
                            "Se conecta al punto de acceso WiFi Direct de otro dispositivo como una " +
                                    "estación WiFi (cliente). Si está conectado a un punto de acceso WiFi 'normal', " +
                                    "esto reemplazará temporalmente su conexión WiFi normal."
                        )
                    }
                }
            )
